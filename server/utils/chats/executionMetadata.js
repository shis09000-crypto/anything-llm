...
function executionMetadata({
  ...
  let resolvedRequestedProtocol = nonEmpty(
    requestedProtocol || metrics.requested_protocol
  );
  let resolvedEffectiveProtocol = nonEmpty(
    effectiveProtocol || metrics.effective_protocol
  );
  const resolvedResponseId = nonEmpty(responseId || metrics.response_id);
  if (resolvedModel && !resolvedRequestedProtocol)
    resolvedRequestedProtocol = resolvedResponseId
      ? "responses"
      : "chat_completions";
  if (resolvedModel && !resolvedEffectiveProtocol)
    resolvedEffectiveProtocol = resolvedResponseId
      ? "responses"
      : "chat_completions";

...
const MAIN_OWNERSHIP_RULES = Object.freeze([
  ...
  ["browser_plane", /^(browser_profiles|browser_sessions|browser_workspaces|browser_tabs|browser_tasks|browser_artifacts|browser_bookmarks|browser_history)$/i],
  ["scheduler", /^scheduled_/i],
  ["sync", /^(?:sync_|athena_sync_).+|^(?:athena_ios_push_tokens|athena_mutation_receipts)$/i],
  ["knowledge_reader", /^reader_/i],
  ["knowledge_ingest", /^(workspace_documents|document_vectors|DocumentIndexStatus|KnowledgeNode|KnowledgeEdge|EdgeEvidence|ConceptChunkMap|GraphExtractionJob|GraphLabelTranslationCache|KnowledgeGraphRepair|KnowledgeGraphEvidenceUsage|KnowledgeNodeMetrics)/],
  ["knowledge_query", /^(GraphRetrievalCache)$/],
  ["model_runtime", /^ai_(price_catalog|budget_|usage_|eval_)/i],
  ["maintenance", /^(account_deletion_|system_patrol_|operations_action_|security_audit_)/i],
]);

const AUTH_OWNERSHIP_RULES = Object.freeze([
  ["identity", /.*/],
]);
...
function ownerForTable(table, database = "main") {
  const rules = database === "auth" ? AUTH_OWNERSHIP_RULES : MAIN_OWNERSHIP_RULES;
...
function runtimeSchemaForRole(role, database = "main") {
  const normalized = String(role || "api").trim().toLowerCase();
  const schemas = database === "auth" ? AUTH_RUNTIME_SCHEMAS : MAIN_RUNTIME_SCHEMAS;
...
async function verifyClientOwnership(client, { database = "main", role } = {}) {
  ...
  if (violations.length) {
    const error = new Error(`module_schema_acl_invalid:${violations.join(",")}`);

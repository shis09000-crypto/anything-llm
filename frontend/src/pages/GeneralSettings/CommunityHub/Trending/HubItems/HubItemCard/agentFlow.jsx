...
export default function AgentFlowHubCard({ item }) {
  ...
  return (
    <Link
      ...
      className="bg-black/70 light:bg-slate-100 rounded-lg p-3 hover:bg-black/60 light:hover:bg-slate-200 motion-hover cursor-pointer group border border-transparent hover:border-slate-400 flex flex-col h-full"
    >
      ...
      <div className="flex justify-end mt-2">
        <Link
          to={paths.communityHub.importItem(item.importId)}
          className="text-primary-button hover:text-primary-button/80 text-sm font-medium px-3 py-1.5 rounded-md bg-black/30 light:bg-slate-200 group-hover:bg-black/50 light:group-hover:bg-slate-300 motion-hover"
        >
          Import →
        </Link>
      </div>

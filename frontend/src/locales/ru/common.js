...
const TRANSLATIONS = {
  onboarding: {
    ...
    llm: {
      ...
      description:
        "Athena может работать с различными провайдерами LLM. Этот сервис будет обеспечивать обработку чата.",
    },
    userSetup: {
    ...
    survey: {
      ...
      description:
        "Помогите нам сделать Athena, созданным с учётом ваших потребностей (необязательно).",
  ...
  agent: {
    ...
    settings: {
      ...
      "intelligent-skill-selection": {
        ...
        description:
          "Позволяет использовать неограниченное количество инструментов и сократить использование токенов до 80% на запрос – Athena автоматически выбирает наиболее подходящие навыки для каждого запроса.",
        "max-tools": {
  ...
  api: {
    title: "API ключи",
    description:
      "API ключи позволяют владельцу программно получать доступ к этому экземпляру Athena и управлять им.",
    ...
    modal: {
      ...
      helper:
        "После создания API-ключ можно использовать для программного доступа к этому экземпляру Athena и его настройки.",
      name: {
        label: "Название",
  ...
  llm: {
    ...
    description:
      "Это учетные данные и настройки для вашего предпочтительного поставщика чата и встраивания LLM. Важно, чтобы эти ключи были актуальными и правильными, иначе Athena не будет работать должным образом.",
  ...
  transcription: {
    ...
    "warn-start":
      "Использование локальной модели whisper на машинах с ограниченной оперативной памятью или процессором может привести к зависанию Athena при обработке медиафайлов.",
    "warn-recommend":
  ...
  embedding: {
    ...
    "desc-end":
      "Встраивание - это процесс превращения текста в векторы. Эти учетные данные необходимы для превращения ваших файлов и подсказок в формат, который Athena может использовать для обработки.",
    provider: {
  ...
  vector: {
    ...
    description:
      "Это учетные данные и настройки для того, как будет функционировать ваш экземпляр Athena. Важно, чтобы эти ключи были актуальными и правильными.",
    provider: {
  ...
  privacy: {
    ...
    description:
      "Это ваша конфигурация для того, как подключенные сторонние поставщики и Athena обрабатывают ваши данные.",
  ...
  connectors: {
    ...
    upload: {
      ...
      "privacy-notice":
        "Эти файлы будут загружены в процессор документов на этом экземпляре Athena. Файлы не отправляются и не передаются третьим лицам.",
    },
    pinning: {
      what_pinning: "Что такое закрепление документа?",
      pin_explained_block1:
        "Когда вы <b>закрепляете</b> документ в Athena, мы вставляем всё его содержимое в окно запроса, чтобы LLM мог полностью его понять.",
      pin_explained_block2:
        "Это работает лучше всего с <b>моделями с большим контекстом</b> или небольшими файлами, критичными для базы знаний.",
      pin_explained_block3:
        "Если по умолчанию ответы Athena вас не удовлетворяют, закрепление — отличный способ получить более качественные ответы одним кликом.",
      accept: "Хорошо, понял",
    },
    watching: {
      what_watching: "Что делает функция наблюдения за документом?",
      watch_explained_block1:
        "Когда вы <b>наблюдаете</b> за документом в Athena, мы <i>автоматически</i> синхронизируем его содержимое с оригинальным источником через регулярные интервалы. Это автоматически обновляет содержимое во всех рабочих пространствах, где используется этот файл.",
  ...
  customization: {
    interface: {
      ...
      description:
        "Настройте свои предпочтения пользовательского интерфейса для Athena.",
    },
    branding: {
      title: "Брендинг и создание продуктов с собственной меткой.",
      description:
        "Настройте свою версию Athena с использованием собственных брендинговых элементов.",
    },
    chat: {
      title: "Чат",
    ...
    items: {
      ...
      "display-language": {
        ...
        description:
          "Выберите предпочитаемый язык для отображения пользовательского интерфейса Athena – когда доступны переводы.",
      },
      logo: {
    ...
    publish: {
      ...
      generic: {
        unauthenticated: {
          ...
          description:
            "Для публикации материалов необходимо сначала пройти аутентификацию в сообществе Athena.",
  ...
  security: {
    ...
    password: {
      ...
      description:
        "Защитите ваш экземпляр Athena паролем. Если вы забудете его, метода восстановления не существует, поэтому убедитесь, что вы сохранили этот пароль.",
  ...
  telegram: {
    ...
    description:
      "Подключите свою инстанцию Athena к Telegram, чтобы вы могли общаться со своими рабочими пространствами с любого устройства.",
    setup: {
      step1: {

...
test("auth bootstrap failure forces the current workspace back to login", async () => {
  const { mod, replacements } = await loadCoordinator(
    "/workspace/alpha?t=bootstrap"
  );
  assert.equal(mod.redirectToLogin({ reason: "force_reauth" }), true);
  assert.equal(replacements.length, 1);
  assert.match(replacements[0], /^\/login\?nt=1&reason=force_reauth/);
  assert.equal(mod.consumeAuthReturnRef(), "/workspace/alpha?t=bootstrap");
});

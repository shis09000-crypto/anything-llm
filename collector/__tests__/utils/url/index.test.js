...
describe("validURL", () => {
  ...
  it("should validate a valid URL", () => {
    ...
    // Hostnames do not require a public suffix, but numeric host shorthand is
    // normalized by URL into 0.0.0.123 and must be rejected by Guard V2.
  ...
  it("should block private, loopback and metadata-class IPs by default", () => {
    ...
    expect(validURL("http://169.254.169.254/latest/meta-data")).toBe(false);
    expect(validURL("http://100.64.0.1")).toBe(false);
    expect(validURL("http://[::1]")).toBe(false);
    expect(validURL("http://[::ffff:127.0.0.1]")).toBe(false);
  });

  it("ignores the legacy global bypass and only permits explicit allowlists", () => {
    process.env.COLLECTOR_ALLOW_ANY_IP = "true";
    expect(validURL("http://192.168.1.1")).toBe(false);
    process.env.COLLECTOR_PRIVATE_NETWORK_ALLOWLIST = "192.168.1.0/24";
    expect(validURL("http://192.168.1.1")).toBe(true);
    expect(validURL("http://192.168.2.1")).toBe(false);
...
describe("validYoutubeVideoUrl", () => {
  ...
  it("returns true for youtube watch URLs with v param", () => {
    ...
    expect(
      validYoutubeVideoUrl(`https://youtube.com/watch?v=${ID}&t=10s`)
    ).toBe(true);
    expect(validYoutubeVideoUrl(`https://m.youtube.com/watch?v=${ID}`)).toBe(
      true
    );
  ...
  it("returns true for embed and v path formats", () => {
    expect(validYoutubeVideoUrl(`https://www.youtube.com/embed/${ID}`)).toBe(
      true
    );
  ...
  it("returns false for non-YouTube hosts", () => {
    expect(
      validYoutubeVideoUrl("https://example.com/watch?v=dQw4w9WgXcQ")
    ).toBe(false);
  ...
  it("returns false for unrelated YouTube paths without a video id", () => {
    expect(
      validYoutubeVideoUrl("https://www.youtube.com/user/somechannel")
    ).toBe(false);
  ...
  it("returns the video ID for valid YouTube video URLs", () => {
    expect(
      validYoutubeVideoUrl(`https://www.youtube.com/watch?v=${ID}`, true)
    ).toBe(ID);
    expect(
      validYoutubeVideoUrl(`https://youtube.com/watch?v=${ID}&t=10s`, true)
    ).toBe(ID);
    expect(
      validYoutubeVideoUrl(`https://m.youtube.com/watch?v=${ID}`, true)
    ).toBe(ID);
    ...
    expect(validYoutubeVideoUrl(`https://youtu.be/${ID}?si=abc`, true)).toBe(
      ID
    );
    expect(validYoutubeVideoUrl(`https://youtu.be/${ID}/extra`, true)).toBe(ID);
    expect(
      validYoutubeVideoUrl(`https://www.youtube.com/embed/${ID}`, true)
    ).toBe(ID);
    expect(validYoutubeVideoUrl(`https://youtube.com/v/${ID}`, true)).toBe(ID);
    // invalid video IDs
    expect(
      validYoutubeVideoUrl(`https://www.youtube.com/watch?v=invalid`, true)
    ).toBe(null);
    expect(
      validYoutubeVideoUrl(`https://youtube.com/watch?v=invalid`, true)
    ).toBe(null);
    expect(
      validYoutubeVideoUrl(`https://m.youtube.com/watch?v=invalid`, true)
    ).toBe(null);
    ...
    expect(validYoutubeVideoUrl(`https://youtu.be/invalid?si=abc`, true)).toBe(
      null
    );
  });
});

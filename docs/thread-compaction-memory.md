...
Target mode uses `THREAD_COMPACTION_CONTEXT_WINDOW_TOKENS` as the thread memory ceiling and derives manual/auto targets from that ceiling. The default manual and auto target ratio is `0.2`, meaning compaction aims to keep the injected thread-state layer at or below 20% of the configured thread-memory budget, roughly an 80% compression target. This is an upper bound, not a requirement to fill the budget; a concise Conversation State Capsule can be much smaller when the durable state is small. The visible thread memory limit is intentionally independent of the active chat model context window, so switching workspace models does not shrink an existing thread from a long-memory budget back to a small model window. Thread compaction resolves through the rough LLM task tier by default (`deepseek-v4-flash`) and does not fall back to the workspace chat model; if the configured compaction provider/model is unavailable, compaction returns an explicit error instead of silently using a larger chat model.
